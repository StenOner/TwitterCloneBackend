...
const userValidation = {
    fullValidate: (req, res, next) => {
        ...
        if (!body.email.match(emailRegex)) return res.status(403).send({ message: 'El email ingresado no es valido.' })
        ...
        if (!(typeof body.state === 'boolean')) return res.status(403).send({ message: 'El estado de usuario es necesario.' })
        next()
    },
    noPasswordValidate: (req, res, next) => {
        ...
        if (!body.email.match(emailRegex)) return res.status(403).send({ message: 'El correo ingresado no es valido.' })
        if (!body.userName || !(body.userName.trim().length > 0)) return res.status(403).send({ message: 'El nombre de usuario es necesario.' })
        next()
    },
    updateEmailValidate: (req, res, next) => {
        ...
        if (!body.newEmail.match(emailRegex)) return res.status(403).send({ message: 'El nuevo correo ingresado no es valido.' })
        next()
    },
    updatePasswordValidate: (req, res, next) => {
        const body = req.body
        if (!body.email.match(emailRegex)) return res.status(403).send({ message: 'El correo no es valido.' })
    ...
    deleteUserValidate: (req, res, next) => {
        const body = req.body
        if (!body.email.match(emailRegex)) return res.status(403).send({ message: 'El correo no es valido.' })
        if (!body.password || !(body.password.trim().length > 0)) return res.status(403).send({ message: 'La clave no es valida.' })
        next()
    }
}
